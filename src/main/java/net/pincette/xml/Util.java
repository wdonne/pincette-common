...

{
  ...
  ancestors(final Node node) {
    return
        takeWhile(node.getParentNode(), Node::getParentNode, n -> n instanceof Element)
            .map(n -> (Element) n);
  }
  ...
  getIndex(final AttributesImpl atts, final String namespaceUri, final String localName) {
    return
        Optional
            .of(atts.getIndex(namespaceUri, localName))
            .filter(i -> i != -1)
            .map(i -> pair(i, atts));
  }
  ...
  public static boolean
  isNameChar(final char c) {
    return
        Character.isLetterOrDigit(c) || c == '.' || c == '-' || c == '_' || c == ':'
            || isCombiningChar(c) || isExtenderChar(c);
  }

  public static boolean
  isName(String s) {
    return
        s.length() > 0 && isNameStartChar(s.charAt(0)) &&
            s.chars().allMatch(c -> isNameChar((char) c));
  }

  public static boolean
  isNameStartChar(final char c) {
    return Character.isLetter(c) || c == '_' || c == ':';
  }

  public static boolean
  isXml(final String mimeType) {
    return
        Optional
            .of(mimeType)
  ...
  stripMimeTypeParameters(final String mimeType) {
    return
        Optional
            .of(mimeType.indexOf(';'))
